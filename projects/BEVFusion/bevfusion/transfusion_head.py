...
# def visualize_masks_and_heatmaps(self, batch_idx, map_mask, original_heatmap, masked_heatmap):
#     """可视化地图掩码和热图
    
#     Args:
#         batch_idx: 批次索引，用于区分不同的样本
#         map_mask: 地图掩码 [C, H, W]
#         original_heatmap: 原始热图 [C, H, W]
#         masked_heatmap: 应用掩码后的热图 [C, H, W]
#     """
#     # 创建保存目录
#     save_dir = os.path.join('visualization', 'query_masks')
#     os.makedirs(save_dir, exist_ok=True)
    
#     # 生成时间戳
#     timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    
#     # 选择几个类别进行可视化（如果热图有多个类别通道）
#     num_classes = original_heatmap.shape[0]
#     classes_to_viz = min(3, num_classes)  # 最多可视化3个类别
    
#     # 获取掩码的单通道版本（如果是多通道）
#     if map_mask.shape[0] > 1:
#         map_mask_single = map_mask.max(dim=0, keepdim=True)[0]
#     else:
#         map_mask_single = map_mask
    
#     plt.figure(figsize=(15, 5 * classes_to_viz))
    
#     for class_idx in range(classes_to_viz):
#         # 为每个类别创建一行3列的子图
#         plt.subplot(classes_to_viz, 3, class_idx * 3 + 1)
#         plt.title(f'Map Mask (Class {class_idx})')
#         plt.imshow(map_mask_single[0].cpu().numpy(), cmap='gray')
#         plt.colorbar()
        
#         plt.subplot(classes_to_viz, 3, class_idx * 3 + 2)
#         plt.title(f'Original Heatmap (Class {class_idx})')
#         plt.imshow(original_heatmap[class_idx].cpu().numpy(), cmap='viridis')
#         plt.colorbar()
        
#         plt.subplot(classes_to_viz, 3, class_idx * 3 + 3)
#         plt.title(f'Masked Heatmap (Class {class_idx})')
#         plt.imshow(masked_heatmap[class_idx].cpu().numpy(), cmap='viridis')
#         plt.colorbar()
    
#     # 调整布局并保存
#     plt.tight_layout()
#     filename = f'mask_heatmap_b{batch_idx}_{timestamp}.png'
#     plt.savefig(os.path.join(save_dir, filename))
#     plt.close()
    
#     print(f"✅ Visualization saved to {os.path.join(save_dir, filename)}")
...
class TransFusionHead(nn.Module):
    ...
    def __init__(
        ...
        self.map_mask_bev = None #map_mask-init-query
    ...
    def visualize_masks_and_heatmaps(self, batch_idx, map_mask, original_heatmap, masked_heatmap):
        ...
        # 选择几个类别进行可视化
        num_classes = original_heatmap.shape[0]
        classes_to_viz = min(10 ,num_classes)
    ...
    def forward_single(self, inputs, metas):
        ...
        # # 在处理 heatmap 部分：
        # # 引导掩码：只保留 map_mask_bev 中为 1 的区域的 heatmap 响应
        # if hasattr(self, 'map_mask_bev') and self.map_mask_bev is not None:
        #     print("✅ Using map_mask_bev in TransfusionHead", self.map_mask_bev.shape)
            
        #     # 确保尺寸匹配
        #     if self.map_mask_bev.shape[-2:] != heatmap.shape[-2:]:
        #         map_mask_resized = F.interpolate(
        #             self.map_mask_bev, 
        #             size=(heatmap.shape[2], heatmap.shape[3]), 
        #             mode='nearest'
        #         )
        #     else:
        #         map_mask_resized = self.map_mask_bev
            
        #     # 确保通道维度匹配
        #     if map_mask_resized.shape[1] == 1 and heatmap.shape[1] > 1:
        #         map_mask_expanded = map_mask_resized.expand(-1, heatmap.shape[1], -1, -1)
        #     else:
        #         map_mask_expanded = map_mask_resized
            
        #     # 应用掩码到 heatmap
        #     print("  ⤷ Expanded mask shape:", map_mask_expanded.shape)
        #     print("  ⤷ Heatmap shape:", heatmap.shape)
        #     heatmap = heatmap * map_mask_expanded
        # else:
        #     print("❌ map_mask_bev not available in TransfusionHead")
        ...
        # 在处理 heatmap 部分：
        original_heatmap = heatmap.clone()  # 保存原始 heatmap

        # 引导掩码：在原始热图基础上增强 map_mask_bev 中为 1 的区域
        map_mask_bev = getattr(self, 'map_mask_bev', None)
        if map_mask_bev is not None:
            print("✅ Using map_mask_bev in TransfusionHead (Residual Enhancement):", map_mask_bev.shape)
            
            # 匹配尺寸
            if map_mask_bev.shape[-2:] != heatmap.shape[-2:]:
                map_mask_bev = F.interpolate(
                    map_mask_bev, 
                    size=(heatmap.shape[2], heatmap.shape[3]), 
                    mode='nearest'
                )
            ...
            # 匹配通道数（为可视化保存原始单通道版本）
            map_mask_single = map_mask_bev.clone()
            
            if map_mask_bev.shape[1] == 1 and heatmap.shape[1] > 1:
                map_mask_bev = map_mask_bev.expand(-1, heatmap.shape[1], -1, -1)
            
            print("  ⤷ Processed mask shape:", map_mask_bev.shape)
            
            # 残差增强: 原始热图 + 掩码增强部分
            # 可以调整增强强度: heatmap + alpha * heatmap * map_mask_bev
            alpha = 0.5  # 增强系数，可以根据需要调整
            enhancement = alpha * heatmap * map_mask_bev
            enhanced_heatmap = heatmap + enhancement
            
            print(f"  ⤷ 增强前热图最大值: {heatmap.max().item():.4f}")
            print(f"  ⤷ 增强后热图最大值: {enhanced_heatmap.max().item():.4f}")
            
            # 随机选择批次中的几个样本进行可视化（避免生成太多图片）
            batch_size = heatmap.shape[0]
            viz_samples = min(2, batch_size)  # 最多可视化2个样本
            
            for b_idx in range(viz_samples):
                # 为当前批次样本可视化
                self.visualize_masks_and_heatmaps(
                    b_idx,
                    map_mask_single[b_idx],  # 单通道掩码
                    original_heatmap[b_idx],  # 原始热图
                    enhanced_heatmap[b_idx]   # 增强后热图
                )
            
            # 更新 heatmap
            heatmap = enhanced_heatmap
